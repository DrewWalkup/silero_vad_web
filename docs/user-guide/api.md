...
| `onEmitChunk`                 | `(audio: Float32Array) => any`                                | `() => {}`                                      | Callback to run everytime framesToEmitMs of audio accumulates in the audio buffer after speech start is detected. Takes as arg a Float32Array with audio samples, sample rate 16000. If speech ends before enough audio accumulate, the remaining audio can be read from onSpeechEnd callback arg. This will not run if  `framesToEmitMs` is 0 (default).                                                                                                       | 
...
| `framesToEmitMs`             | `number`                                                                           | `0`                                                | [see algorithm configuration](algorithm.md#configuration)                                                                                                                                                                                                                                                  |
...
| `model`                   | `"v5"`, `"v6"`, or `"legacy"`                                                      | `"legacy"`                                           | which Silero VAD model to use                                                                                                                                                                                                                                                                              |
| `baseAssetPath`           | `string`                                                                           | `/`                                                  | URL or path relative to webroot where `vad.worklet.bundle.min.js`, `silero_vad_legacy.onnx`, `silero_vad_v5.onnx`, and `silero_vad_v6.onnx` will be loaded from                                                                                                                                            |
